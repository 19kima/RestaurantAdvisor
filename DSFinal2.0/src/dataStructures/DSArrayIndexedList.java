...
public class DSArrayIndexedList<T> extends DSArrayUnsortedList<T> implements DSIndexedListInterface<T>, Serializable{
	public DSArrayIndexedList() {
		super();
	}

	public DSArrayIndexedList(int origCap) {
		super(origCap);
	}
	...
	// elements at that index or higher have 1 added to their index.
	{
		if ((index < 0) || (index > size())) {
			throw new IndexOutOfBoundsException(
					"illegal index of " + index + " passed to ArrayIndexedList add method.\n");
		}
	...
	// returns the replaced element.
	{
		if ((index < 0) || (index >= size())) {
			throw new IndexOutOfBoundsException(
					"illegal index of " + index + " passed to ArrayIndexedList set method.\n");
		}
		T hold = list[index];
		list[index] = element;
		return hold;
	...
	// Otherwise, returns the element on this list at position index.
	{
		if ((index < 0) || (index >= size())) {
			throw new IndexOutOfBoundsException(
					"illegal index of " + index + " passed to ArrayIndexedList get method.\n");
		}
		return list[index];
	}
	...
	// higher than that index have 1 subtracted from their index.
	{
		...
		if ((index < 0) || (index >= size())) {
			throw new IndexOutOfBoundsException(
					"illegal index of " + index + " passed to ArrayIndexedList remove method.\n");
		}
